...
plot = ggplot(marker_means_long) +geom_tile(aes(Marker,Cluster,fill=Exp)) + scale_fill_gradient(low="white",high="orangered3") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+ 
  theme(axis.text.x = element_text(angle = 65, hjust=1)) + xlab("") + labs(fill="Exp") +
