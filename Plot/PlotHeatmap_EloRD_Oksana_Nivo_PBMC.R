...
celltype = c("SELL+ CD14+ Mono","TGFb1+ CD14+ Mono","CD14+ CD16+ Mono","CD16+ Mono","IFN+ Mono", 'CCL3+ Mono')
celltype_factor = c("SELL+ CD14+ Monocytes","TGFb1+ CD14+ Monocytes","CD14+ CD16+ Monocytes","CD16+ Monocytes","IFN+ Monocytes",'CCL3+ Monocytes')
...
Ident_list[Ident_list == "CCL3+ Mono"] <- "CCL3+ Monocytes"
...
markers <- c("CD14","SELL","S100A8", "S100A9", "S100A12",
             "HLA-DRA","HLA-DRB1","TIMP1","TGFB1","CD300E","LGALS3",
             "FCGR3A","CSF1R","MS4A7","CD74",
             "MX1","ISG15","IFI6","IFI44L", # IFN+ Mono
             'CCL3', 'IL1B','CCL3L3','CXCL2', 'CXCL8') # CCL3+ Mono
marker_counts <- mat[,colnames(mat) %in% markers]
marker_counts$ident <- Ident_list
marker_means <- aggregate(scale(marker_counts[, 1:(ncol(marker_counts)-1)]), list(marker_counts$ident), mean)
rownames(marker_means) = marker_means$Group.1
