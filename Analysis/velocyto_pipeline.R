...
data_input <- RunVelocity(object = data_input, deltaT = 1, kCells = 25, fit.quantile = 0.02)
ident.colors <- (scales::hue_pal())(n = length(x = levels(x = data_input)))
names(x = ident.colors) <- levels(x = data_input)
cell.colors <- ident.colors[Idents(object = data_input)]
names(x = cell.colors) <- colnames(x = data_input)
show.velocity.on.embedding.cor(emb = Embeddings(object = data_input, reduction = "umap"), 
                               vel = Tool(object = data_input, 
